...
Authentasaurus uses the following terms: Area, Group and Permission
...
At login, authentasaurus would load the user's group permissions into his/her session and then attempts to read that hash
when it meats a require helper on a controller.
...
require_write: requires the user to have a write permission to that area to access the actions specified
...
require_read: requires the user to have a read permission to that area to access the actions specified
